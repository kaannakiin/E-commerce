...
import CancelOrderButton from "../../hesabim/siparislerim/_components/CancelOrderButton";
...
export type OrderItemsForRefund = Prisma.OrderItemsGetPayload<{
  select: {
    id: true;
    price: true;
    quantity: true;
...
const OrderDetailPage = async (params: { params: Params }) => {
  ...
  if (order.userId) {
    ...
    if (!session) {
      redirect(
        `/giris?callbackUrl=${encodeURIComponent(`/hesabim/siparislerim/${slug}`)}`,
      );
    } else {
      redirect(`/hesabim/siparislerim/${slug}`);
    }
  }
