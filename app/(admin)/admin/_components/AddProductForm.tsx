...
const AddProductForm = ({ feedCat }) => {
  ...
  return (
    <div className="flex flex-col gap-10 py-5 lg:px-10">
      <div className="flex w-full flex-col gap-5 lg:flex-row">
        <div className="flex flex-col lg:w-1/4">
          <h1 className="text-2xl font-bold uppercase">Genel Bilgiler</h1>
          <form onSubmit={productHandleSubmit(onProductSubmit)}>
            ...
            />
            <NumberInput
              {...productRegister("taxPrice", { setValueAs: (v) => Number(v) })}
              onChange={(e) => productSetValue("taxPrice", Number(e))}
              min={0}
              max={100}
              label="Kdv oranı"
              size="sm"
